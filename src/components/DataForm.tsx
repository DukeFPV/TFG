...
export default function DataForm() {
  ...
  const [selectedDate, setSelectedDate] = useState<Dayjs | null>(dayjs('1990-01-31'));
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (isLoaded && isSignedIn && user) {
        ...
        const res = await fetch(`/api/get-profile?email=${encodeURIComponent(userEmail)}`);
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Enviar los datos a traves de la API
        ...
        <div className="flex flex-col ml-7 sm:ml-0 mb-5 sm:flex-row sm:justify-evenly">
          <div className="sm:flex sm:flex-col sm:mb-5">
            ...
            <div className="flex flex-col mb-5">
              ...
              <select className="border-2 border-purple-300 max-w-60 py-1 pl-2 rounded-xl" {...register("Género")}>
