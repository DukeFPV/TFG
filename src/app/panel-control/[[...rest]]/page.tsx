...
export default async function PanelControl() {
  ...
  const latestConversations = await db.select().from(messages).orderBy(desc(messages.createdAt)).limit(5);
  const totalConversations = await db.select({ count: messages.id }).from(messages);
